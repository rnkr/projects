...
# Imputation ---- Address NA values in Age
  ...
  # datasettest$Age[is.na(datasettest$Age)] <- mean(datasettest$Age, na.rm = TRUE ) 
   ...
   # Fare not available for only one row in test data  
   datasettest$Fare[is.na(datasettest$Fare)] <- mean(datasettest$Fare, na.rm = TRUE )

  # TRAIN - decision trees for imputation
  anova_mod <- rpart(Age ~ Pclass + Sex + SibSp + Parch + Fare + Embarked + famly, data=datasetorig[!is.na(datasetorig$Age), ], method="anova", na.action=na.omit)  
  datasetorig$Age[is.na(datasetorig$Age)] <-  predict(anova_mod, datasetorig[is.na(datasetorig$Age), ])
  
  # TEST
  anova_mod <- rpart(Age ~ Pclass + Sex + SibSp + Parch + Fare + Embarked + famly, data=datasettest[!is.na(datasettest$Age), ], method="anova", na.action=na.omit)  
  datasettest$Age[is.na(datasettest$Age)] <-  predict(anova_mod, datasettest[is.na(datasettest$Age), ])
...
datasetvalid <- dataset[-cutoffindex, ]
...
# fittune.rf <- train(Survived ~., data = dataset, method = "rf", metric = metric,tuneGrid = tunegrid, trControl = trainControl )
...
predictions <- predict( fittune.rf, datasetvalid )
confusionMatrix(predictions, datasetvalid$Survived)


# run prdictions on full dataset
predictionfull <- predict (fittune.rf, datasetorig)
confusionMatrix(predictionfull, datasetorig$Survived)


# test dataset
predictionsontest <- predict( fittune.rf, datasettest )
submit <- data.frame(PassengerId = datasettest$PassengerId, Survived = predictionsontest)
write.csv(submit, file = "TitanicP2.csv", row.names = FALSE)
